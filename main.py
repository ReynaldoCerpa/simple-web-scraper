    ...
    while len(reviews) < num_reviews:
        ...
        for i in range(offset, len(review_containers)):
            try:
                ...
                reviews.append({'user': user, 'comment': comment, 'date': date_published, 'images': number_of_images, 'stars': stars})
...
def save_reviews_to_csv(raw_business_name, reviews):
    ...
    with open(filename, 'w', encoding='utf-8') as output:
        ...
        writer.writerow(['usuario', 'Comentario', 'Fecha', '# Imagenes', 'Calificacion'])
        for item in reviews:
            writer.writerow([item['user'], item['comment'], item['date'], item['images'], item['stars']])
